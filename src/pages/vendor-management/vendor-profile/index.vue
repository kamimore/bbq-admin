...
const { uniq, cloneDeep, uniqBy } = require("lodash");
...
export default {
    ...
    methods: {
        ...
        async approveRequests({ items, comment }) {
            ...
            if (this.isSourcingHead && isApproved) {                
                await this.$http.updateById("vendor", this.vendor.id, {
                    status: "approved_by_sourcing_head",                    
                });
            } else if (this.isSourcingHead) {
                await this.$http.updateById("vendor", this.vendor.id, {
                    status: "rejected_by_sourcing_head",
                });
            } else if (isApproved) {
                await this.$http.updateById("vendor", this.vendor.id, {
            ...
            switch (this.currentView) {
                ...
                case "location-detail-section":
                    return {
                        ...
                        mstStates: uniqBy(this.mstStates, 'code'),
                    };
