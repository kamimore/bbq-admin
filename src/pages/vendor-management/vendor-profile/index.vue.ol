...
export default {
    ...
    methods: {
        ...
        async updateStatus({ status, comment }) {
            try {
                ...
                let vendorStatus = status == "approved" ? "approved_by_mdm" : "rejected_by_mdm";
    ...
    computed: {
        ...mapGetters("account", ["isSuperAdmin", "isVendor", "isMDM", "isSourcingRole", "isRegionalSourcing"]),
        currentProp() {
            switch (this.currentView) {
                ...
                case "vendor-detail-section":
                    return {
                        ...
                        mode: this.isMDM && this.vendor.status === "approved_by_sourcing" ? "add" : "readOnly",
                    };
        ...
        isEditable() {
            ...
            if (this.isMDM && this.vendor.status != "approved_by_mdm") {
        ...
        showWaitingMsg() {
            return ["approved_by_mdm", "rejected_by_mdm", "rejected_by_sourcing"].includes(this.vendor.status);
        },
        vendorTypeTermMode() {
            // if (this.mode === "add") {
            if (this.isVendor && ["draft", "approved_by_mdm"].includes(this.vendor.status)) {
                return "add";
